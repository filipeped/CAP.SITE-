...
function isDuplicateEvent(eventId: string): boolean {
  ...
  return false; // ✅ CORREÇÃO: Retorno explícito - não é duplicata
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const filteredData = req.body.data.filter((event: any) => {
      // ✅ CORREÇÃO CRÍTICA: Usar eventId existente ou gerar apenas se não existir
      if (!event.event_id) {
    ...
    // 🌐 FORMATAÇÃO OTIMIZADA: IP no formato preferido pela Meta (definido antes do loop)
    ...
    const enrichedData = filteredData.map((event: any) => {
      ...
      // ✅ CORREÇÃO: EventId já foi gerado/validado no filtro de deduplicação
